...
async function checkApiAvailability() {
  try {
    const res = await fetch(`${API_BASE}/scraper/jobs/?cif=${FARMEC_CIF}&rows=1`, {
...
describe('Integration: API Workflow', () => {
  ...
  describe('ANAF API', () => {
    ...
    itIfAnaf('should search for FARMEC brand and find the company', async () => {
      ...
      const farmec = results.find(c =>
        c.name.toUpperCase().includes('FARMEC') && c.statusLabel === 'Funcțiune'
      );
      expect(farmec).toBeDefined();
      expect(farmec.cui.toString()).toBe(COMPANY_CIF);
    }, 15000);
    ...
    itIfAnaf('should use cached data when API fails (getCompanyFromANAFWithFallback)', async () => {
      const cached = { cui: 199150, name: COMPANY_NAME };
  ...
  describe('API Jobs Core', () => {
    ...
    itIfApi('should query jobs by CIF and return valid data', async () => {
      ...
      if (result.numFound === 0) {
        console.log('No FARMEC jobs in API — skipping job field assertions (scraper may not have run yet)');
        return;
      }
  ...
  describe('Full Validation Workflow', () => {
    ...
    itIfAnaf('should complete the ANAF → Peviitor validation path', async () => {
      ...
      const farmecCompany = searchResults.find(c =>
        c.name.toUpperCase().includes('FARMEC') && c.statusLabel === 'Funcțiune'
      );
      expect(farmecCompany).toBeDefined();

      const anafData = await anaf.getCompanyFromANAF(farmecCompany.cui.toString());
    ...
    itIfApi('should validate company and query API for existing jobs', async () => {
      ...
      if (companyResult.existingJobsCount === 0) {
        console.log('No FARMEC jobs in API — skipping job count assertion (scraper may not have run yet)');
        return;
      }
