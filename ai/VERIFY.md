...
4. Verifică în SOLR că job-urile companiei apar:
   ```bash
   # CIF-ul companiei
   curl --user "${(no credentials needed — peviitor API)}" "https://solr.peviitor.ro/solr/job/select?q=cif:199150&rows=10"
   ```
5. Verifică că `docs/jobs.md` a fost generat și este accesibil:
   - https://peviitor-scrapers.github.io/farmec-sa-nodejs-scraper/jobs.md
